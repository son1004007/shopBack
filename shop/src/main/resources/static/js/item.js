$(document).ready(function() {
	...
	$("#itemReadBtn").on("click", function() {
		$.ajax({
			...
			success : function(data) {
				...
				var str = "<img src='items/display?itemId=" + data.itemId + "&tiemstamp=" + new Data().getTime() + "' width ='210' height='240'>";
				
				$("#preview").append(str);
				$("#preview2").empty();
				
				var str2 = "<img src='items/preview?itemId=" + data.itemId + "&timestamp=" + new Data().getTime() + "' width='210' height='240'>";
	...
	$("#itemModifyBtn").on("click", function() {
		...
		$.ajax({
			...
			dataType: "text",
			headers : {
				"Authoorization" : "Bearer " + ACCESS_TOKEN
			},
			processData: false,
			contentType: false,
			type: "PUT",
	...
	$("#itemDownloadBtn").on("click", function() {
		...
		req.onload = function (event) {
			...
			alert(responseHedaer);
			
			var downloadFilename = responseHeader.substring(22, responseHeaeder.length -1);
			
			var decodedDownloadFilename = decodeURIComponent(escape(downlaodFilename));
